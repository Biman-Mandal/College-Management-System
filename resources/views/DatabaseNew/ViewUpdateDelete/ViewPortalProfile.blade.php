...
@section('content')
<title>|| View Portal Profile ||</title>
	   ...
	   @endif
		<div class="ViewDataP ViewData">
			<table>
				...
				@foreach($PortalDataP as $Data)
				<tr class="Header">
					<td>{{$Data->Name}}</td>
					<td>{{$Data->Depertment}}</td>
					<td>{{$Data->UniversityRoll}}</td>
					<td>{{$Data->MoblileNumber}}</td>
					<td>{{$Data->Email}}</td>
					<td>
						<a href="#">View</a>
						<br>
						<a href="#">Update</a>
						<br>
						<a href={{"DeletePPData/".$Data->id}}>Delete</a>
					</td>
